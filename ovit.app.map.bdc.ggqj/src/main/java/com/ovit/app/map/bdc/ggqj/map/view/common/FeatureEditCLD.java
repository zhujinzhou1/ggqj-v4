package com.ovit.app.map.bdc.ggqj.map.view.common;
...
public class FeatureEditCLD extends FeatureEdit {

    final static String TAG = "FeatureEditCLD";

    public FeatureEditCLD(){ super();}
    public FeatureEditCLD(MapInstance mapInstance, Feature feature) {
...
//        requiredFileds.addAll(Arrays.asList(new String [] {"JZWMC","ZDDM","BDCDYH","ZRZH","ZCS","ZH"}));
    ...
    @Override
    public void build() {
        LinearLayout v_feature = (LinearLayout) LayoutInflater.from(activity).inflate(R.layout.app_ui_ai_aimap_feature_cld, v_content);
        try {
            if (feature != null) {
                fillView(v_feature);
            }
