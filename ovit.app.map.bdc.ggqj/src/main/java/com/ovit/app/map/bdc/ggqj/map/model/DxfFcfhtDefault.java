...
public class DxfFcfhtDefault {
    ...
    public DxfFcfhtDefault(MapInstance mapInstance) {
        this.mapInstance = mapInstance;
        set(mapInstance.aiMap.getProjectWkid());
    }

    public DxfFcfhtDefault set(String dxfpath) {
        this.dxfpath = dxfpath;
        return this;
    }

    public DxfFcfhtDefault set(SpatialReference spatialReference) {
        this.spatialReference = spatialReference;
        return this;
    }

    public DxfFcfhtDefault set(Feature f_bdc, Feature f_h, Feature f_zd, List<Feature> fs_zrz, List<Feature> fs_c_all) {
    ...
    public DxfFcfhtDefault write() throws Exception {
        getExtend(); // 多大范围
        if (dxf == null) {
            dxf = new DxfAdapter();
            // 创建dxf
        ...
        try {
            {
                ...
                dxf.write(cel_3_2, null,FeatureHelper.Get(f,"MPH",""), o_fontsize, o_fontstyle, false, DxfHelper.COLOR_CYAN, 0);
                // 单元格3-3
                x_ = x_ + w * 4 / 15;
...
//                dxf.write(mapInstance,fs_all);
                ...
                dxf.writeH(mapInstance, f, "", null, lineLabel, d_move, DxfHelper.LINE_LABEL_INNER, DxfHelper.LINE_WIDTH_3, DxfHelper.TYPE_NEIMENG);
    ...
    private double getTnmj(Feature f, List<Feature> fs_all) {
        if (f != null) {
            String tableName = f.getFeatureTable().getTableName();
            if (FeatureHelper.TABLE_NAME_H.equals(tableName)) {
                return FeatureHelper.Get(f, "SCJZMJ", 0.00);
            } else {
                double tnmj = 0.0;
                for (Feature feature : fs_all) {
                    String mTableName = feature.getFeatureTable().getTableName();
                    if (mTableName.equals(FeatureHelper.TABLE_NAME_H)) {
                        tnmj += FeatureHelper.Get(feature, "SCJZMJ", 0.00);
                    }
                }
                return tnmj;
            }
    ...
    public DxfFcfhtDefault save() throws Exception {
        if (dxf != null) {
            dxf.save();
        }
        return this;
