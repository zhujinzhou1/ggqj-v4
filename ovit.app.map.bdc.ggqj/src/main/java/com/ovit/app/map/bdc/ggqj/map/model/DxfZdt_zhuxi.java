...
public class DxfZdt_zhuxi {
    ...
    public DxfZdt_zhuxi(MapInstance mapInstance) {
        this.mapInstance = mapInstance;
        set(mapInstance.aiMap.getProjectWkid());
    }

    public DxfZdt_zhuxi set(String dxfpath) {
        this.dxfpath = dxfpath;
        return this;
    }

    public DxfZdt_zhuxi set(SpatialReference spatialReference) {
        this.spatialReference = spatialReference;
        return this;
    }

    public DxfZdt_zhuxi set(Feature f_zd, List<Feature> fs_zd, List<Feature> fs_zrz, List<Feature> fs_z_fsjg, List<Feature> fs_h_fsjg
    ...
    public DxfZdt_zhuxi write() throws Exception {
        getExtend(); // 多大范围
        if (dxf == null) {
            dxf = new DxfAdapter();
            // 创建dxf
...
//            dxf.write(new Envelope(envelope.getCenter(), p_width + o_split * 3, p_height + o_split * 5)); // 图框
            ...
            dxf.writeText(p_title, "宗地草图", o_fontsize*2, DxfHelper.FONT_WIDTH_DEFULT, o_fontstyle, 0, 1, 2, DxfHelper.COLOR_BYLAYER, null, null);

            Point p_unit = new Point(envelope.getXMax() , envelope.getYMax() + o_split*0.5, envelope.getSpatialReference());
            dxf.writeText(p_unit, "单位：m·㎡ ", o_fontsize, DxfHelper.FONT_WIDTH_DEFULT, o_fontstyle, 0, 2, 2, DxfHelper.COLOR_BYLAYER, null, null);
            double w = p_width; //行宽
...
//                dxf.write(cel_1_1,"不动产单元号");
            dxf.write(cel_1_1, null, "宗地编号", o_fontsize, null, false, DxfHelper.COLOR_BYLAYER, 0);
            // 单元格1-2
            x_ = x_ + w * 2 / 15;
            ...
            dxf.write(cel_2_1, null, "地籍图号", o_fontsize, null, false, DxfHelper.COLOR_BYLAYER, 0);
            // 单元格2-2
            x_ = x_ + w * 2 / 15;
            ...
            for (Feature f_zd : fs_zd) {
                if (!FeatureHelper.Get(f_zd,FeatureHelper.TABLE_ATTR_ZDDM,"").contains(zddm)) {
                    ...
                    float ft = 0.5f;
                    dxf.writeText(new Point(p.getX(), p.getY() + 1 * ft), FeatureHelper.Get(f_zd, "QLRXM", ""), ft, DxfHelper.FONT_WIDTH_DEFULT, "", 0, 1, 1, DxfHelper.COLOR_RED, "JZD", "302004");
...
//            DxfHelper.writeN(new Point(p_n.getX(), p_n.getY() - o_split, p_n.getSpatialReference()), o_split,dxf);
            ...
            dxf.writeText(p_blc, "1:200", o_fontsize, DxfHelper.FONT_WIDTH_DEFULT, o_fontstyle, 0, 1, 2, DxfHelper.COLOR_BYLAYER, null, null);

            Point p_chr = new Point(envelope.getCenter().getX() + w * 1 / 3+w*1/10, envelope.getYMin() - o_split * 0.3, envelope.getSpatialReference());
            dxf.writeText(p_chr, "测绘员："+ GsonUtil.GetValue(mapInstance.aiMap.JsonData,"HZR",""), o_fontsize, DxfHelper.FONT_WIDTH_DEFULT, o_fontstyle, 0, 1, 2, DxfHelper.COLOR_BYLAYER, null, null);
            Point p_shr = new Point(envelope.getCenter().getX() + w * 1 / 3+w*1/10, envelope.getYMin() - 3*o_split * 0.3, envelope.getSpatialReference());
            dxf.writeText(p_shr, "审核员："+ GsonUtil.GetValue(mapInstance.aiMap.JsonData,"SHR",""), o_fontsize, DxfHelper.FONT_WIDTH_DEFULT, o_fontstyle, 0, 1, 2, DxfHelper.COLOR_BYLAYER, null, null);
    ...
    public DxfZdt_zhuxi save() throws Exception {
        if (dxf != null) {
            dxf.save();
        }
        return this;
