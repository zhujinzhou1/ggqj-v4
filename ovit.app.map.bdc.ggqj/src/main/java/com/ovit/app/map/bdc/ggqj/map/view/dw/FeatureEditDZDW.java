...
public class FeatureEditDZDW extends FeatureEdit {

    final static String TAG = "FeatureEditDZDW";

    public FeatureEditDZDW(){ super();}
    public FeatureEditDZDW(MapInstance mapInstance, Feature feature) {
...
//        requiredFileds.addAll(Arrays.asList(new String [] {"JZWMC","ZDDM","BDCDYH","ZRZH","ZCS","ZH"}));
    ...
    @Override
    public void build() {
        LinearLayout v_feature = (LinearLayout) LayoutInflater.from(activity).inflate(R.layout.app_ui_ai_aimap_feature_dzdw, v_content);
        try {
            if (feature != null) {
                fillView(v_feature);
            }
