...
public class FeatureEditMZDW extends FeatureEdit {

    final static String TAG = "FeatureEditMZDW";

    public FeatureEditMZDW(){ super();}
    public FeatureEditMZDW(MapInstance mapInstance, Feature feature) {
...
//        requiredFileds.addAll(Arrays.asList(new String [] {"JZWMC","ZDDM","BDCDYH","ZRZH","ZCS","ZH"}));
    ...
    @Override
    public void build() {
        LinearLayout v_feature = (LinearLayout) LayoutInflater.from(activity).inflate(R.layout.app_ui_ai_aimap_feature_mzdw, v_content);
        try {
            if (feature != null) {
                fillView(v_feature);
            }
