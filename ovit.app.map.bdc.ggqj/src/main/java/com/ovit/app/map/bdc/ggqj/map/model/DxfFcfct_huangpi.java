...
public class DxfFcfct_huangpi {
    ...
    public DxfFcfct_huangpi(MapInstance mapInstance) {
        this.mapInstance = mapInstance;
        set(mapInstance.aiMap.getProjectWkid());
    }

    public DxfFcfct_huangpi set(String dxfpath) {
        this.dxfpath = dxfpath;
        return this;
    }

    public DxfFcfct_huangpi set(SpatialReference spatialReference) {
        this.spatialReference = spatialReference;
        return this;
    }

    public DxfFcfct_huangpi set(String bdcdyh, Feature f_zd, List<Feature> fs_zrz, List<Feature> fs_z_fsjg, List<Feature> fs_h, List<Feature> fs_h_fsjg) {
        this.bdcdyh = bdcdyh;
        this.f_zd = f_zd;
        this.fs_zrz=fs_zrz;
    ...
    public DxfFcfct_huangpi write() throws Exception {
        getExtend(); // 多大范围
...
//                Point p_chr = new Point(envelope.getCenter().getX() + w * 1 / 3+w*1/10, envelope.getYMin() - o_split * 0.3, envelope.getSpatialReference());
    ...
    public DxfFcfct_huangpi save() throws Exception {
        if (dxf != null) {
            dxf.save();
        }
        return this;
    ...
    private void writeC(List<C> cs ,List<String[]> list_z, int index ,int page, Envelope envelope)throws Exception{
        ...
        if(index<=cs.size()) {
            ...
            if (fs!=null&&fs.size()>0){
             dxf.write(mapInstance, fs, null, DxfHelper.getDistanceMove(fs,envelope,envelope.getSpatialReference()), DxfHelper.TYPE_TIANMEN, DxfHelper.LINE_LABEL_OUTSIDE,new LineLabel());
            }
        }
    }
