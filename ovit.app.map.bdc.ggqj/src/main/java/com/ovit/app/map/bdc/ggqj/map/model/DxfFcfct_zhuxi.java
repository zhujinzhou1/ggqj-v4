...
public class DxfFcfct_zhuxi {
    ...
    public DxfFcfct_zhuxi(MapInstance mapInstance) {
        this.mapInstance = mapInstance;
        set(mapInstance.aiMap.getProjectWkid());
    }

    public DxfFcfct_zhuxi set(String dxfpath) {
        this.dxfpath = dxfpath;
        return this;
    }

    public DxfFcfct_zhuxi set(SpatialReference spatialReference) {
        this.spatialReference = spatialReference;
        return this;
    }

    public DxfFcfct_zhuxi set(String bdcdyh, Feature f_zd, List<Feature> fs_zrz, List<Feature> fs_z_fsjg, List<Feature> fs_h, List<Feature> fs_h_fsjg) {
        this.bdcdyh = bdcdyh;
        this.f_zd = f_zd;
        this.fs_zrz=fs_zrz;
...
//            o_fontsize = (float) (o_fontsize*scale);
    ...
    public DxfFcfct_zhuxi write() throws Exception {
        getExtend(); // 多大范围
        ...
        try {
           ...
           Map<Integer,String []> map_z=new HashMap<>();
            ...
            for (Feature f_zrz : fs_zrz) {
                ...
                z[3] = String.format("%.2f", FeatureHelper.Get(f_zrz, "SCJZMJ", 0.00d));
...
//              dxf.write(new Envelope(envelope.getCenter(), p_width + o_split * 3, p_height + o_split * 5)); // 图框
                ...
                Point p_n_1=new Point(envelope.getXMax() - p_width /2  - o_split*0.75,envelope.getYMax() - o_split*0.75);
                dxf.write(p_n_1, DxfHelper.LINETYPE_SOLID_LINE, "北", o_fontsize*1.6f, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);
                DxfHelper.writeN(new Point(p_n_1.getX(), p_n_1.getY() - o_split*0.75, p_n_1.getSpatialReference()), o_split*0.75,dxf);

                Point p_n_2=new Point(envelope.getXMax() - p_width /2  - o_split*0.75,envelope.getYMax() - p_height*0.65f - o_split*0.75);
                dxf.write(p_n_2, DxfHelper.LINETYPE_SOLID_LINE, "北", o_fontsize*1.6f, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);
                DxfHelper.writeN(new Point(p_n_2.getX(), p_n_2.getY() - o_split*0.75, p_n_2.getSpatialReference()), o_split*0.75,dxf);
...
//                writeN(new Point(p_n.getX(), p_n.getY() - o_split, p_n.getSpatialReference()), o_split, -alpha);
                ...
                // 1  楼
                x_ = x_ + w / 12;
                Envelope cel_4_6_2 = new Envelope(x_, y_, x_ + w / 12-w/900, y_ - 2 * h, p_extend.getSpatialReference());
                dxf.write(cel_4_6_2, DxfHelper.LINETYPE_SOLID_LINE, "1", o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);

                // 2  楼
                x_ = x_ + w / 12-w/900;
                Envelope cel_4_6_3 = new Envelope(x_, y_, x_ + w / 12-w/900, y_ - 2 * h, p_extend.getSpatialReference());
                dxf.write(cel_4_6_3, DxfHelper.LINETYPE_SOLID_LINE, "2", o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);

                // 3  楼
                x_ = x_ + w / 12-w/900;
                Envelope cel_4_6_4 = new Envelope(x_, y_, x_ + w / 12-w/900, y_ - 2 * h, p_extend.getSpatialReference());
               ...
               /* x_ = x_ + w / 20-w/900;
                ...
                x_ = x_ + w / 18+w / 180;
                Envelope cel_4_6_8 = new Envelope(x_, y_, x_ + w / 18, y_ - 2 * h, p_extend.getSpatialReference());
                ...
                dxf.write(cel_4_11, DxfHelper.LINETYPE_SOLID_LINE, "（5）", o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);

                y_ = y_ - h;
                Envelope cel_4_12 = new Envelope(x_, y_, x_ + w / 12, y_ - h, p_extend.getSpatialReference());
                dxf.write(cel_4_12, DxfHelper.LINETYPE_SOLID_LINE, "（9）", o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);
...
//                    }
                    for (int j = 0; j < 3; j++) {
                        cel_i_j = new Envelope(x_, y_, x_ + w / 12-w/900, y_ - h, p_extend.getSpatialReference());
                        if (z != null) {
                            try {
                                if (j == 0) {
                    ...
                    dxf.write(cel_i_j_5, DxfHelper.LINETYPE_SOLID_LINE, z==null?"":z[3], o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);

                    x_ = x_ + w / 18+w/180;
                    Envelope cel_i_j_6 = new Envelope(x_, y_, x_ + w / 18, y_ -  h, p_extend.getSpatialReference());
                    dxf.write(cel_i_j_6, DxfHelper.LINETYPE_SOLID_LINE, z==null?"":z[4], o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);
                   ...
                   /* x_ = x_ + w / 36;
                    ...
                    dxf.write(cel_i_j_8, DxfHelper.LINETYPE_SOLID_LINE, z==null?"":z[5], o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);
                }
                ...
                x_ = x + w/2+w/12;
                y_=y_-h;
                Envelope cel_10_2 = new Envelope(x_, y_, x_+ w/12-w/900, y_ - h, p_extend.getSpatialReference());
                //cs.get(1).getJzmj()+""
                dxf.write(cel_10_2, null, page==1?cjzmj(cs,1):"", o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);

                // 单元格4-4 框   二层面积汇总
                x_ = x_ + w/12-w/900;
                Envelope cel_10_3 = new Envelope(x_, y_, x_+ w/12-w/900, y_ - h, p_extend.getSpatialReference());
                dxf.write(cel_10_3, null, page==1?cjzmj(cs,2):"", o_fontsize, fontstyle, false, DxfHelper.COLOR_BYLAYER, 0);

                // 单元格4-5 框   三层面积汇总
                x_ = x_ + w/12-w/900;
                Envelope cel_10_4 = new Envelope(x_, y_, x_+ w/12-w/900, y_ - h, p_extend.getSpatialReference());
               ...
               /* x_ = x_ + w/20-w/900;
                ...
                // 单元格4-9 框   结构
                x_ = x_ + w/18+w/180;
                Envelope cel_10_8 = new Envelope(x_, y_, x_+ w/18, y_ - h, p_extend.getSpatialReference());
...
//                dxf.writeText(p_blc, "1:200", o_fontsize*1.5f, o_fontstyle, 0, 1, 2, DxfHelper.COLOR_BYLAYER, null, null);
    ...
    public DxfFcfct_zhuxi save() throws Exception {
        if (dxf != null) {
            dxf.save();
        }
        return this;
